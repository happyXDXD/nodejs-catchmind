...
module.exports = function (io) {
  ...
  gameIO.on("connection", (socket) => {
    ...
    socket.on("register", (data) => {
      ...
      targetRoom.users.set(data.username.trim(), socket.id);
      targetRoom.userScore.set(data.username.trim(), 0);
    ...
    socket.on("change-player", () => {
      ...
      // 해당 인덱스의 플레이어 이름을 가져와서 next_player_name에 할당
      next_player_name = Array.from(targetRoom.users.keys())[
        info.current_index
      ];
      gameIO.to(roomID).emit("next-player-name", next_player_name);
    });
